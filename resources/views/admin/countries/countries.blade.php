...
@section('title', $translations['sidebar_nav_countries'] ?? 'Countries')
@section('script', url('public/js/dashboard/country.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_countries'] ?? 'Countries'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_country" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="countries_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-country-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_countries'] ?? 'Countries'}}</h5>
                            {{-- <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="cny_CountryName" id="cny_CountryName" class="form-control icon_control">
                            </div> --}}
                            <input type="hidden" id="pkCny">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="cny_CountryName_{{$v->language_key}}" id="cny_CountryName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_genitive_name'] ?? 'Genitive Name'}}</label>
                                <input type="text" name="cny_CountryNameGenitive" id="cny_CountryNameGenitive" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_dative_name'] ?? 'Dative Name'}}</label>
                                <input type="text" name="cny_CountryNameDative" id="cny_CountryNameDative" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_adjective_name'] ?? 'Adjective Name'}}</label>
                                <input type="text" name="cny_CountryNameAdjective" id="cny_CountryNameAdjective" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="cny_Note" id="cny_Note" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_status'] ?? 'Status'}} *</label>
                                <select class="form-control icon_control dropdown_control" name="cny_Status" id="cny_Status">
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/country.js') }}"></script>
@endpush
