@section('title','Foreign Language Group')
@section('script', url('public/js/dashboard/foreign_language_group.js')) 
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_course_groups'] ?? 'Course Groups'}} > </span>{{$translations['sidebar_nav_flg'] ?? 'Foreign Language Group'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_foreignLanguageGroup" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="foreignLanguageGroup_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-foreignLanguageGroup-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_flg'] ?? 'Foreign Language Group'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="fon_Uid" id="fon_Uid" value="SBE" class="form-control icon_control">
                                <input type="hidden" id="pkFon">
                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="fon_Name_{{$v->language_key}}" id="fon_Name_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach 
                            <div class="form-group">
                                <label>{{$translations['gn_notes'] ?? 'Notes'}}</label>
                                <input type="text" name="fon_Notes" id="fon_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" src="{{ url('public/js/dashboard/foreign_language_group.js') }}"></script>
@endpush
