...
@section('title', $translations['sidebar_nav_religions'] ?? 'Religions')
@section('script', url('public/js/dashboard/religion.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_religions'] ?? 'Religions'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_religion" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="religions_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-religion-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_religions'] ?? 'Religions'}}</h5>
                            {{-- <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="rel_ReligionName" id="rel_ReligionName" class="form-control icon_control">
                            </div> --}}
                            <input type="hidden" id="pkRel">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="rel_ReligionName_{{$v->language_key}}" id="rel_ReligionName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_adjective_name'] ?? 'Name Adjective'}}</label>
                                <input type="text" name="rel_ReligionNameAdjective" id="rel_ReligionNameAdjective" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="rel_Notes" id="rel_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/religion.js') }}"></script>
@endpush
