@section('title','Student Behaviour')
@section('script', url('public/js/dashboard/student_behaviour.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_student_masters'] ?? 'Student Masters'}} > </span>{{$translations['sidebar_nav_student_behaviour'] ?? 'Student Behaviour'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_studentBehaviour" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="studentBehaviour_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-studentBehaviour-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_student behaviour'] ?? 'Student Behaviour'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="sbe_Uid" id="sbe_Uid" value="SBE" class="form-control icon_control">
                                <input type="hidden" id="pkSbe">

                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="sbe_BehaviourName_{{$v->language_key}}" id="sbe_BehaviourName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="sbe_Notes" id="sbe_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/student_behaviour.js') }}"></script>
@endpush
