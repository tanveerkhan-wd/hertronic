...
@section('title', $translations['sidebar_nav_postal_code'] ?? 'Postal Code')
@section('script', url('public/js/dashboard/postal_code.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_postal_code'] ?? 'Postal Code'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_postalCode" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-md-5 text-md-right mb-3">
                <div class="row">
                    <div class="col-4 text-right pr-0 pt-1">
                        <label class="blue_label">{{$translations['gn_town'] ?? 'Town'}}</label>
                    </div>
                    <div class="col-8">
                        <select id="town_filter" class="form-control without_border icon_control dropdown_control">
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="postalCode_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
                                    <th>{{$translations['gn_name'] ?? 'Name'}}</th>
                                    <th>{{$translations['sidebar_nav_postal_code'] ?? 'Postal Code'}}</th>
                                    <th>{{$translations['gn_town'] ?? 'Town'}}</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-postal-code-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_postal_code'] ?? 'Postal Code'}}</h5>
                            {{-- <div class="form-group">
                                <input type="text" name="pof_PostOfficeName" id="pof_PostOfficeName" class="form-control icon_control">
                            </div> --}}
                            <input type="hidden" id="pkPof">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="pof_PostOfficeName_{{$v->language_key}}" id="pof_PostOfficeName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['sidebar_nav_postal_code'] ?? 'Postal Code'}} *</label>
                                <input type="text" name="pof_PostOfficeNumber" id="pof_PostOfficeNumber" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_town'] ?? 'Town'}} *</label>
                                <select class="form-control icon_control dropdown_control" name="fkPofMun" id="fkPofMun">
                                    <option value="">{{$translations['gn_select'] ?? 'Select'}}</option>
                                    @foreach($data as $k =>$v)
                                        <option value="{{$v->pkMun}}">{{$v->mun_MunicipalityName}}</option>
                                    @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="pof_Notes" id="pof_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" src="{{ url('public/js/dashboard/postal_code.js') }}"></script>
@endpush