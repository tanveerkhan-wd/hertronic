...
@section('title', $translations['sidebar_nav_ownership_types'] ?? 'Ownership Types')
@section('script', url('public/js/dashboard/ownership_type.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_ownership_types'] ?? 'Ownership Types'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_ownership_type" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="ownership_type_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-ownership-type-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_ownership_types'] ?? 'Ownership Types'}}</h5>
                            <input type="hidden" id="pkOty">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="oty_OwnershipTypeName_{{$v->language_key}}" id="oty_OwnershipTypeName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <!-- <div class="form-group">
                                <label>Name *</label>
                                <input type="text" name="oty_OwnershipTypeName_en" id="oty_OwnershipTypeName_en" class="form-control icon_control">
                                <input type="hidden" id="pkOty">
                            </div> -->
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="oty_Notes" id="oty_Notes" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_status'] ?? 'Status'}} *</label>
                                <select class="form-control icon_control dropdown_control" name="oty_Status" id="oty_Status">
...
<script type="text/javascript" src="{{ url('public/js/dashboard/ownership_type.js') }}"></script>
@endpush