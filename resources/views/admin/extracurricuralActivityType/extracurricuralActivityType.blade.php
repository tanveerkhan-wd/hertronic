@section('title','Extracurricural Activity Type')
@section('script', url('public/js/dashboard/extracurricural_activity_type.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_student_masters'] ?? 'Student Masters'}} > </span>{{$translations['sidebar_nav_ecat'] ?? 'Extracurricural Activity Type'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_extracurricuralActivityType" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="extracurricuralActivityType_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-extracurricuralActivityType-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_ecat'] ?? 'Extracurricural Activity Type'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="sat_Uid" id="sat_Uid" value="SBE" class="form-control icon_control">
                                <input type="hidden" id="pkSat">

                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="sat_StudentExtracurricuralActivityName_{{$v->language_key}}" id="sat_StudentExtracurricuralActivityName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="sat_Notes" id="sat_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/extracurricural_activity_type.js') }}"></script>
@endpush
