@section('title','General Purpose Group')
@section('script', url('public/js/dashboard/general_purpose_group.js')) 
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_course_groups'] ?? 'Course Groups'}} > </span>{{$translations['sidebar_nav_gpg'] ?? 'General Purpose Group'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_generalPurposeGroup" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="generalPurposeGroup_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-generalPurposeGroup-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_gpg'] ?? 'General Purpose Group'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="gpg_Uid" id="gpg_Uid" value="SBE" class="form-control icon_control">
                                <input type="hidden" id="pkGpg">

                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="gpg_Name_{{$v->language_key}}" id="gpg_Name_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach 
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="gpg_Notes" id="gpg_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" src="{{ url('public/js/dashboard/general_purpose_group.js') }}"></script>
@endpush
