@section('title', $translations['sidebar_nav_education_periods'] ?? 'Education Period')
@section('script', url('public/js/dashboard/educationPeriod.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_education_periods'] ?? 'Education Period'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_educationPeriod" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="educationPeriod_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
                                    <th>{{$translations['gn_name'] ?? 'Name'}}</th>
                                    <th>{{$translations['gn_adjective_name'] ?? 'Name Adjective'}}</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-educationPeriod-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_education_periods'] ?? 'Education Period'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="edp_Uid" id="edp_Uid" value="EDP" class="form-control icon_control">
                                <input type="hidden" id="pkEdp">

                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="edp_EducationPeriodName_{{$v->language_key}}" id="edp_EducationPeriodName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_adjective_name'] ?? 'Name Adjective'}} *</label>
                                <input type="text" name="edp_EducationPeriodNameAdjective" id="edp_EducationPeriodNameAdjective" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/educationPeriod.js') }}"></script>
@endpush
