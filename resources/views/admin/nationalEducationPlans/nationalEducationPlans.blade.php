...
@section('title', $translations['sidebar_nav_national_education_plan'] ?? 'National Education Plan')
@section('script', url('public/js/dashboard/national_education_plan.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_national_education_plan'] ?? 'National Education Plan'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_education_plan" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="education_plan_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
            ...
            <div class="modal-body">
                ...
                <form name="add-education-plan-form">
                    <div class="row">
                        ...
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_national_education_plan'] ?? 'National Education Plan'}}</h5>
                            {{-- <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="nep_NationalEducationPlanName" id="nep_NationalEducationPlanName" class="form-control icon_control">
                            </div> --}}
                            <input type="hidden" id="pkNep">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="nep_NationalEducationPlanName_{{$v->language_key}}" id="nep_NationalEducationPlanName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="nep_Notes" id="nep_Notes" class="form-control icon_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_status'] ?? 'Status'}} *</label>
                                <select class="form-control icon_control dropdown_control" name="nep_Status" id="nep_Status">
...
<script type="text/javascript" src="{{ url('public/js/dashboard/national_education_plan.js') }}"></script>
@endpush