@section('title','Facultative Courses Group')
@section('script', url('public/js/dashboard/facultative_courses_group.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_course_groups'] ?? 'Course Groups'}} > </span>{{$translations['sidebar_nav_fcg'] ?? 'Facultative Courses Group'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_facultativeCoursesGroup" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="facultativeCoursesGroup_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-facultativeCoursesGroup-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_fcg'] ?? 'Facultative Courses Group'}}</h5>
                            <div class="form-group">
                                <input type="hidden" name="fcg_Uid" id="fcg_Uid" value="SBE" class="form-control icon_control">
                                <input type="hidden" id="pkFcg">

                            </div>
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="fcg_Name_{{$v->language_key}}" id="fcg_Name_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach 
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="fcg_Notes" id="fcg_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" src="{{ url('public/js/dashboard/facultative_courses_group.js') }}"></script>
@endpush
