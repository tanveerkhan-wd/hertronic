...
@section('title', $translations['sidebar_nav_cantons'] ?? 'Cantons')
@section('script', url('public/js/dashboard/canton.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_masters'] ?? 'Masters'}} > </span>{{$translations['sidebar_nav_cantons'] ?? 'Cantons'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_canton" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            <div class="col-md-3 text-md-right mb-3">
                <div class="row">
                    ...
                    <div class="col-8">
                        <select onchange="fetchState(this.value,1)" id="country_filter" class="form-control without_border icon_control dropdown_control">
            ...
            <div class="col-md-3 text-md-right mb-3">
                <div class="row">
                    <div class="col-4 text-right pr-0 pt-1">
                        <label class="blue_label">{{$translations['gn_state'] ?? 'State'}}</label>
                    </div>
                    <div class="col-8">
                        <select id="state_filter" class="form-control without_border icon_control dropdown_control">
                            <option value="">{{$translations['gn_select'] ?? 'Select'}}</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="cantons_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
                                    <th>{{$translations['gn_name'] ?? 'Name'}}</th>
                                    <th>{{$translations['gn_state'] ?? 'State'}}</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-canton-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_cantons'] ?? 'Cantons'}}</h5>
                            {{-- <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="can_CantonName" id="can_CantonName" class="form-control icon_control">
                            </div> --}}
                            ...
                            <div class="form-group">
                                ...
                                <select onchange="fetchState(this.value,2)" class="form-control icon_control dropdown_control" name="selCountry" id="selCountry">
                                    <option value="">{{$translations['gn_select'] ?? 'Select'}}</option>
                                    @foreach($data as $k =>$v)
                                        <option value="{{$v->pkCny}}">{{$v->cny_CountryName}}</option>
                                    @endforeach
                            ...
                            <div class="form-group">
                                <label>{{$translations['gn_state'] ?? 'State'}} *</label>
...
<script type="text/javascript" src="{{ url('public/js/dashboard/canton.js') }}"></script>
@endpush
