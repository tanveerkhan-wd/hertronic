...
@section('title','Class')
@section('script', url('public/js/dashboard/class.js'))
@section('content')
 <!-- Page Content  -->
<div class="section">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span> @if(Auth::guard('admin')->user()->type=='MinistryAdmin') {{$translations['sidebar_nav_masters'] ?? 'Masters'}} @else {{$translations['sidebar_nav_ministry_masters'] ?? 'Ministry Masters'}} @endif > </span>{{$translations['sidebar_nav_classes'] ?? 'Classes'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_class" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="classes_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-class-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_classes'] ?? 'Classes'}}</h5>
                            <!-- <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="cla_ClassName" id="cla_ClassName" class="form-control icon_control">
                            </div> -->
                            <input type="hidden" id="pkCla">
                            @foreach($languages as $k => $v)
                                <div class="form-group">
                                    <label>{{$translations['gn_name'] ?? 'Name'}} {{$v->language_name}} *</label>
                                    <input type="text" name="cla_ClassName_{{$v->language_key}}" id="cla_ClassName_{{$v->language_key}}" class="form-control force_require icon_control" required="">
                                </div>
                            @endforeach
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="cla_Notes" id="cla_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/class.js') }}"></script>
@endpush
