...
@section('title', $translations['gn_school'] ?? 'School')
    ...
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>@if(Auth::guard('admin')->user()->type=='HertronicAdmin') {{$translations['sidebar_nav_ministry_masters'] ?? 'Ministry Masters'}} > @endif </span>{{$translations['sidebar_nav_school_management'] ?? 'School Management'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_school" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            ...
            @if(Auth::guard('admin')->user()->type=='MinistryAdmin')
            <div class="col-md-2 col-6 mb-3">
                <a class="ajax_request no_sidebar_active" data-slug="admin/addSchool" href="{{url('/admin/addSchool')}}"><button class="theme_btn full_width small_btn">{{$translations['gn_add_new'] ?? 'Add New'}}</button></a>
            </div>
            @endif
        </div>
        <div class="row">
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="school_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
                                    <th>{{$translations['gn_name'] ?? 'Name'}}</th>
                                    <th>{{$translations['gn_ministry_license_number'] ?? 'Ministry License Number'}}</th>
                                    <th>{{$translations['gn_school_coordinator'] ?? 'School Coordinator'}}</th>
