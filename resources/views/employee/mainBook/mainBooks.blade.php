    ...
    <div class="container-fluid">
        <div class="row ">
            <div class="col-12 mb-3">
                <h2 class="title"><span>{{$translations['sidebar_nav_school'] ?? 'School'}} > </span>{{$translations['sidebar_nav_main_books'] ?? 'Main Books'}}</h2>
            </div>   
            <div class="col-md-4 mb-3">
                <input type="text" id="search_mainbook" class="form-control without_border icon_control search_control" placeholder="{{$translations['gn_search'] ?? 'Search'}}">
            </div>  
            <div class="col-md-4 text-md-right mb-3">
                <div class="row">
                    <div class="col-4 text-right pt-1 p-0">
                        <label class="blue_label">{{$translations['gn_opening_date'] ?? 'Opening Date'}}</label>
                    </div>
                    <div class="col-8">
                        <input type="text" id="date_filter" class="form-control datepicker date_control icon_control">
                    </div>
                </div>
            </div> 
            ...
            <div class="col-12">
                <div class="theme_table">
                    <div class="table-responsive">
                        <table id="mainbook_listing" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>UID</th>
                                    <th>{{$translations['gn_name'] ?? 'Name'}}</th>
                                    <th>{{$translations['gn_opening_date'] ?? 'Opening Date'}}</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form name="add-mainbook-form">
                    <div class="row">
                        <div class="col-lg-1"></div>
                        <div class="col-lg-10">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$translations['sidebar_nav_main_books'] ?? 'Main Books'}}</h5>
                            <div class="form-group">
                                <label>{{$translations['gn_name'] ?? 'Name'}} *</label>
                                <input type="text" name="mbo_MainBookNameRoman" id="mbo_MainBookNameRoman" class="form-control icon_control">
                            </div>
                            <input type="hidden" id="pkMbo">
                            <input type="hidden" id="fkMboSch" value="{{$mainSchool}}">
                            <div class="form-group">
                                <label>{{$translations['gn_opening_date'] ?? 'Opening Date'}} *</label>
                                <input type="text" name="mbo_OpeningDate" id="mbo_OpeningDate" class="form-control icon_control datepicker date_control">
                            </div>
                            <div class="form-group">
                                <label>{{$translations['gn_note'] ?? 'Note'}}</label>
                                <input type="text" name="mbo_Notes" id="mbo_Notes" class="form-control icon_control">
                            </div>
...
<script type="text/javascript" id="datatable_script" src="{{ url('public/js/dashboard/mainbook.js') }}"></script>
@endpush
